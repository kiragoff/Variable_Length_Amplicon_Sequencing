...
```{r}
write.csv(out1,file="qc_check.csv")
```
...
We will combine this with your other sample table downstream.

```{r}
getN1 <- function(x) sum(getUniques(x))
track1 <- cbind(out, sapply(dadaF1, getN1), sapply(dadaR1, getN1), sapply(merger1, getN1),
    rowSums(seqtab1))
...
Another set of code for a set of samples from another sequencing run!




Tell R where to find your second set of raw data and make sure the output makes sense.
```{r}
path <- "/mnt/work-drive/kira/its/data2" ##CHANGE THIS to the folder with your second set of raw data
list.files(path)
```

Read the files in the raw data folder into R, and use file names to generate sample names. This assumes you renamed your files as directed before beginning. 

Reminders: Each sample name must be unique, and the only underscores are in _R2_ and _R2_.
```{r}
#Define your forward (_R1_001) and reverse (_R2_001) reads
...
fnF2.filtN <- file.path(path, "filtN", basename(fnF2)) # Put N-filtered files in filtN/ subdirectory
fnR2.filtN <- file.path(path, "filtN", basename(fnR2))
filterAndTrim(fnF2, fnF2.filtN, fnR2, fnR2.filtN, maxN = 0, multithread = TRUE)
...
rbind(FWD.ForwardReads = sapply(FWD.orients, primerHits, fn = fnF2.filtN[[1]]), FWD.ReverseReads = sapply(FWD.orients,
    primerHits, fn = fnR2.filtN[[1]]), REV.ForwardReads = sapply(REV.orients, primerHits,
    fn = fnF2.filtN[[1]]), REV.ReverseReads = sapply(REV.orients, primerHits, fn = fnR2.filtN[[1]]))
```

If the primers are in the correct orientation, continue! If a very small number are wrong, ignore them! If a large number are wrong, change the orientation in the cutadapt section.

```{r}
#Remove primers and do basic QC
path.cut <- file.path(path, "cutadapt")
if(!dir.exists(path.cut)) dir.create(path.cut)
fnF2.cut <- file.path(path.cut, basename(fnF2))
fnR2.cut <- file.path(path.cut, basename(fnR2))

FWD.RC <- dada2:::rc(FWD)
REV.RC <- dada2:::rc(REV)

# Trim FWD and the reverse-complement of REV off of R1 (forward reads)
R2.flags <- paste("-g", FWD, "-a", REV.RC) 
# Trim REV and the reverse-complement of FWD off of R2 (reverse reads)
R2.flags <- paste("-G", REV, "-A", FWD.RC) 
# Run Cutadapt
for(i in seq_along(fnF2)) {
  system2(cutadapt, args = c(R1.flags, R2.flags, "-n", 2, # -n 2 required to remove FWD and REV from reads
                             "--minimum-length=50", "--discard-untrimmed", # remove reads that are short, junky or untrimmed
                             "--cores 0", #lets cutadapt autodetect how many cores are available for use
                             "-o", fnF2.cut[i], "-p", fnR2.cut[i], # output files
                             fnF2.filtN[i], fnR2.filtN[i])) # input files
}

#Check the first sample to make sure primers were properly removed. If there's a dozen or so primers left, ignore them. 
rbind(FWD.ForwardReads = sapply(FWD.orients, primerHits, fn = fnF2.cut[[1]]), FWD.ReverseReads = sapply(FWD.orients,
    primerHits, fn = fnR2.cut[[1]]), REV.ForwardReads = sapply(REV.orients, primerHits,
    fn = fnF2.cut[[1]]), REV.ReverseReads = sapply(REV.orients, primerHits, fn = fnR2.cut[[1]]))
```


```{r}
#Read in files and assign sample names
# Forward and reverse fastq filenames have the format:
cutF2 <- sort(list.files(path.cut, pattern = "_R1_001.fastq.gz", full.names = TRUE))
cutR2 <- sort(list.files(path.cut, pattern = "_R2_001.fastq.gz", full.names = TRUE))

# Extract sample names, assuming filenames have format:
get.sample.name <- function(fname) strsplit(basename(fname), "_")[[1]][1]
sample.names <- unname(sapply(cutF2, get.sample.name))
head(sample.names)

#Assign output file names
filtF2 <- file.path(path.cut, "filtered", basename(cutF2))
filtR2 <- file.path(path.cut, "filtered", basename(cutR2))
```

Do a full QC and look at the number of reads lost. All samples analyzed together should use the same filterAndTrim parameters. 
```{r}
out <- filterAndTrim(cutF2, filtF2, cutR2, filtR2, maxN = 0, maxEE = c(3, 6), truncQ = 2,
    minLen = 50, rm.phix = TRUE, compress = TRUE, multithread = TRUE)  # on windows, set multithread = FALSE
head(out)
...
Okay, this particular data set has truly terrible read quality, and looses a TON of reads here, no matter what I do. The only thing that gets a lot more reads through is increasing the truncQ to 10 (which means we lose fewer reads to the maxEE filter), but that the length of those reads is going to be allllll over the place. Let's try running through the rest with teh data from truncQ = 2 / maxEE = c(2,6) and maxEE = c(2, 4), truncQ = 10

Take a look at the new quality profiles.
```{r}
plotQualityProfile(filtF2[1:4])
plotQualityProfile(filtR2[1:4])
```

Optionally, write and look at a CSV of the filterAndTrim results
```{r}
write.csv(out,file="qc_check2_truncQ2_maxEE_2-6.csv")
```

```{r}
#Dereplicate your data
...
Merge your forward and reverse reads to get the full ITS sequences, then save a sequence table.
```{r}
merger2 <- mergePairs(dadaF2, derepF2, dadaR2, derepR2, verbose=TRUE)
head(merger2[[1]])

seqtab2 <- makeSequenceTable(merger2)
saveRDS(seqtab2, "seqtab2_truncQ10_maxEE_2-4.Rds")
```

```{r}

table(nchar(getSequences(seqtab2))) ##Print size distribution of sequences after trimming
```

```{r}
getN <- function(x) sum(getUniques(x))
track <- cbind(out, sapply(dadaF2, getN), sapply(dadaR2, getN), sapply(merger2, getN),
    rowSums(seqtab2))
...
We don't run fungal samples very often in the Gieg lab, so it's a really good idea to check the database version. 
